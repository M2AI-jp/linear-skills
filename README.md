...
2026年9月時点で、通常CLIの受付アプリ試行により、計画・誤テスト訂正・中断・要求変更・別セッション再開・別ブラウザでの利用・再起動後の保存・統合・ローカル配布を確認しました。Desktopでは限定した再開・保守等を確認しています。同じ全経路をDesktop単独で検証した結果や、任意のモデル・業種での成功率、時間・tokenの削減を保証するものではありません。状態規則はモデルが解釈して実行するため、機械的な遷移強制はありません。
