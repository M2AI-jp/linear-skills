...
状態と受入の規則は[共通契約](skills/linear/references/contract.md)が所有します。このスキル本体にはHookを同梱していません。外部結果と必要な受入はCodexが確認します。
...
この導入経路ではHook用Python、linear-api、隔離判定CLI、Hookの信頼設定は不要です。旧Hookの復活を導入条件にしません。既存のこのパッケージのHookを撤去する場合は、Linear用イベント定義と専用ファイルだけを除き、他用途のHookや設定は保持します。
...
通常の依頼だけで必要なスキルを使用する起動制御は、スキル本体の受入・採用と本開発での使用確認を経て、最終段階でHookも含めて設計・検証・配布します。具体方式と導入手順はその受入後に案内します。上記のHookなし試験は、この起動制御の合格を示すものではありません。
